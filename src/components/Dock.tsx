...
const DOCK_ITEMS = [
  ...
  { id: "skills", icon: Icons.Zap, label: "Tech Arsenal", href: "#skills" },
  { id: "blog", icon: Icons.BookOpen, label: "Insights", href: "#blog" },
  { id: "contact", icon: Icons.Mail, label: "Inbound", href: "#contact" },
  { id: "resume", icon: Icons.FileText, label: "Resume", href: "/Abarnesh_Resume_.docx", external: true },
];
...
export default function Dock() {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex h-16 items-end gap-3 rounded-2xl border border-white/10 bg-black/40 px-4 pb-3 backdrop-blur-xl shadow-[0_0_30px_rgba(0,0,0,0.5)]"
    >
      {DOCK_ITEMS.map((item) => (
  ...
  return (
    <Link href={item.href} target={item.external ? "_blank" : undefined}>
      <motion.div
        ref={ref}
        style={{ width }}
        className="aspect-square rounded-xl bg-white/[0.03] border border-white/5 flex items-center justify-center hover:bg-cyan-500/10 hover:border-cyan-500/30 transition-all group relative duration-300"
      >
        <item.icon className="w-[45%] h-[45%] text-gray-400 group-hover:text-cyan-400 group-hover:drop-shadow-[0_0_8px_rgba(34,211,238,0.5)] transition-all duration-300" />

        {/* Floating Label */}
        <AnimatePresence>
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md text-cyan-400 text-[10px] font-mono px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-cyan-500/20 tracking-widest uppercase">
            {item.label}
          </div>
        </AnimatePresence>

        {/* Indicator Dot */}
        <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-cyan-500/0 group-hover:bg-cyan-400 group-hover:shadow-[0_0_5px_#22d3ee] transition-all duration-300" />
      </motion.div>
    </Link>
  );
}
